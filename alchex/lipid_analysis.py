...
def find_vesicle_leaflets(universe, headgroups="name P*", mda_selection=True):
	points = universe.select_atoms(headgroups)
	universe_resids = {x.id:idx for idx, x in enumerate(universe.residues)}
	coordinates = points.coordinates()
	graph = nx.Graph()
	...
	subgraphs = subgraphs[-2:]
	# sort by radius
	subgraphs=sorted(subgraphs, key= lambda x : coordinates.take(x, axis=0).std(axis=0).mean())
	# inner, outer
	leaflets = [
		[points[x].resid for x in sg.nodes()] for sg in subgraphs
	]
